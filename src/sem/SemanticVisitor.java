...
public class SemanticVisitor extends BabyDuckBaseVisitor<String> {
    ...
    public String visitFuncion(BabyDuckParser.FuncionContext ctx) {
        ...
        // Marca inicio de su cuerpo
        funcStartQuad.put(funcName, quadGen.nextQuad());
        visit(ctx.body());  // genera sus cuádruplos
    ...
    public String visitParametros(BabyDuckParser.ParametrosContext ctx) {
        for (int i = 0; i < ctx.tipo().size(); i++) {
            ...
            functionDirectory.addParam(type);
    ...
    public String visitAssign(BabyDuckParser.AssignContext ctx) {
        ...
        quadGen.generateAssignment(name);
        return null;
    }
    ...
    public String visitFactor(BabyDuckParser.FactorContext ctx) {
        if (ctx.ID()!=null) {
            ...
            if (tp==null) throw new RuntimeException("Var no decl: "+id);
    ...
    public String visitF_call(BabyDuckParser.F_callContext ctx) {
        ...
        if (ctx.expresion()!=null) {
            for (var ec : ctx.expresion()) {
                String t= visit(ec);
                String a= quadGen.popLastOperand();
                quadGen.generateParam(a, idx++);
            }
        }
        Integer target = funcStartQuad.get(fn);
        if (target==null) throw new RuntimeException("Función no decl: "+fn);
        quadGen.generateGoSub(fn, target);
        return functionDirectory.getFunctionReturnType(fn);
    }

    public FunctionDirectory getFunctionDirectory(){ return functionDirectory; }
    public QuadrupleGenerator getQuadrupleGenerator(){ return quadGen; }
    public VirtualMemoryManager getMemoryManager(){ return memory; }
}
